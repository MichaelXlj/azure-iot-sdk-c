...
static BUFFER_HANDLE send_http_workload_request(HSM_CLIENT_HTTP_EDGE* hsm_client_http_edge, STRING_HANDLE uri_path, BUFFER_HANDLE json_to_send)
{
    int result;
    ...
    else if (send_request_to_edge_workload(http_handle, http_headers_handle, STRING_c_str(uri_path), json_to_send, &workload_context) != 0)
    {
...
int hsm_client_http_edge_sign_data(HSM_CLIENT_HANDLE handle, const unsigned char* data, size_t data_len, unsigned char** signed_value, size_t* signed_len)
{
    ...
    else
    {
        ...
        else if ((http_response = send_http_workload_request(hsm_client_http_edge, uri_path, json_to_send)) == NULL)
        {
...
const char* hsm_client_http_edge_get_trusted_certificates(HSM_CLIENT_HANDLE handle)
{
    ...
    else
    {
        ...
        else if ((http_response = send_http_workload_request(hsm_client_http_edge, uri_path, NULL)) == NULL)
        {
            LogError("send_http_workload_request failed");
            trusted_certificates = NULL;
        }
