...
MOCKABLE_FUNCTION(, char*, hsm_client_get_trusted_certificates, HSM_CLIENT_HANDLE, handle);
...
static const HSM_CLIENT_HTTP_EDGE_INTERFACE test_http_edge_interface =
{
    ...
    hsm_client_destroy,
    hsm_client_sign_data,
    hsm_client_get_trusted_certificates
};
#endif
...
static char* my_hsm_client_get_trusted_certificates(HSM_CLIENT_HANDLE handle)
{
    (void)handle;
...
BEGIN_TEST_SUITE(iothub_auth_client_ut)
    ...
    TEST_SUITE_INITIALIZE(suite_init)
    {
        ...
        REGISTER_GLOBAL_MOCK_HOOK(hsm_client_get_trusted_certificates, my_hsm_client_get_trusted_certificates);
        REGISTER_GLOBAL_MOCK_FAIL_RETURN(hsm_client_get_trusted_certificates, NULL);
    ...
    static void setup_iothub_device_auth_generate_credentials_mocks(bool base64_encode_signature, bool urlencode_token_scope)
    {
...
TEST_FUNCTION(IoTHubClient_Auth_Get_TrustedCertificates_succeed)
{
    //arrange
    set_expected_calls_for_device_auth_create_http_edge();
    IOTHUB_SECURITY_HANDLE xda_handle = iothub_device_auth_create();
    ...
    STRICT_EXPECTED_CALL(hsm_client_get_trusted_certificates(IGNORED_PTR_ARG));
        
    //act
    char* result = iothub_device_auth_get_trusted_certificates(xda_handle);
...
// IoTHubClient_Auth_Get_TrustedCertificates only supports Edge based auth.  Verify that others fail.
TEST_FUNCTION(IoTHubClient_Auth_Get_TrustedCertificates_unsupported_authtype_fail)
{
    //arrange
    STRICT_EXPECTED_CALL(gballoc_malloc(IGNORED_NUM_ARG));
    STRICT_EXPECTED_CALL(iothub_security_type()).SetReturn(IOTHUB_SECURITY_TYPE_X509);
